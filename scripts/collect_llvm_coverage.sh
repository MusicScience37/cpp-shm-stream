...
for a in $(find $DIR/bin/shm_stream_test_* $DIR/lib/libshm_streamd.so.*); do
    opts="$opts -object $a"
done
llvm-cov show -ignore-filename-regex='(tests|.conan)/*' -instr-profile=$DIR/coverage/coverage.profdata $opts \
    >$DIR/coverage/coverage.txt
llvm-cov show -ignore-filename-regex='(tests|.conan)/*' -instr-profile=$DIR/coverage/coverage.profdata $opts \
    -format=html -output-dir=$DIR/coverage/html

# output in lcov's format
llvm-cov export -ignore-filename-regex='(tests|.conan)/*' -instr-profile=$DIR/coverage/coverage.profdata $opts \
    -format=lcov > $DIR/coverage/coverage

# output summary to stdout
llvm-cov report -ignore-filename-regex='(tests|.conan)/*' -instr-profile=$DIR/coverage/coverage.profdata $opts |
