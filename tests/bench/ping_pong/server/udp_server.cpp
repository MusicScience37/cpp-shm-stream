...
static boost::asio::ip::udp::endpoint server_endpoint(protocol_type protocol) {
    switch (protocol) {
    case protocol_type::udp_v4:
        return boost::asio::ip::udp::endpoint(
            boost::asio::ip::address_v4::loopback(), udp_port());
    case protocol_type::udp_v6:
        return boost::asio::ip::udp::endpoint(
            boost::asio::ip::address_v6::loopback(), udp_port());
    default:
        std::abort();
    }
}
...
void udp_server::async_receive_next() {
    socket_.async_receive_from(
        boost::asio::buffer(buffer_.data(), buffer_.size()), sender_endpoint_,
...
void udp_server::on_receive(std::size_t bytes_transferred) {
    socket_.send_to(
        boost::asio::const_buffer(buffer_.data(), bytes_transferred),
        sender_endpoint_);
    async_receive_next();
}
